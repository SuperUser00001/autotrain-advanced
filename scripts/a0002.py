...
class MyDataset(Dataset):
    def __init__(self, x_path, y_path, mask_path, cls_idx, reg_idx):
        self.X = np.load(x_path)  # (BatchSize, 9, 3)
    ...
    def __getitem__(self, idx):
        x = torch.tensor(self.X[idx], dtype=torch.float32)  # (9, 3)
...
class MyModel(nn.Module):
    ...
    def forward(self, x):
        ######################## Bgn: 根据模型层定义自行搭建模型 ########################
        feat = self.backbone(x)   # (batch, imbed_dim)
...
class TransformerBackbone(nn.Module):
    def __init__(self, input_shape=(9,3), imbed_dim=128, nhead=4, num_layers=2, dim_feedforward=256):
        super().__init__()
        self.H, self.W = input_shape  # 9行 × 3列
        self.seq_len = self.H * self.W

        # 输入映射：把原始标量 → imbed_dim 向量
    ...
    def forward(self, x):
        ...
        """
        b, h, w = x.shape
        assert h == self.H and w == self.W, f"输入尺寸不匹配: {x.shape}, 需要 {(self.H, self.W)}"

        # (b, h, w, 1) → (b*h*w, 1) → embedding → (b, h*w, imbed_dim)
        x = x.unsqueeze(-1)                          # (b, h, w, 1)
        x = self.embedding(x)                        # (b, h, w, imbed_dim)

        # 加 2D 位置编码
        x = x + self.row_embed[:, :h, :, :] + self.col_embed[:, :, :w, :]
        x = x.view(b, h*w, -1)                       # (b, seq_len=27, imbed_dim)

        # Transformer 编码
        x = self.transformer(x)                      # (b, seq_len, imbed_dim)

        # 池化 → (b, imbed_dim)
        x = x.transpose(1, 2)                        # (b, imbed_dim, seq_len)
