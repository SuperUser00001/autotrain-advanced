...
class TransformerBackbone(nn.Module):
    def __init__(self, input_shape=(9,3,6), embed_dim=128, nhead=4, num_layers=2, dim_feedforward=256):
        ...
        self.k = self.C // 2
        self.seq_len = self.H * self.W * self.k   # 每个探测器当 token

        # 数值部分 embedding：每个探测器值是标量 → embed_dim
        self.value_embedding  = nn.Linear(1, embed_dim)
        # 缺失 embedding：0/1 → embed_dim
        self.mask_embedding = nn.Embedding(2, embed_dim)
        # 探测器索引 embedding (第几个探测器)
        self.detector_embedding = nn.Embedding(self.k, embed_dim)
    ...
    def forward(self, x):
        ...
        # 拆分数值和mask_x
        values = x[:, :, :, :self.k].reshape(b, h*w*self.k, 1)
        mask = x[:, :, :, self.k:].reshape(b, h*w*self.k)
        # 数值embedding
        val_emb = self.value_embedding(values)    # (b, h*w*self.k, embed_dim)

        # 缺失embedding
        mask_emb = self.mask_embedding(mask.long()) # (b, h*w*self.k, embed_dim)

        # detector id embedding
        detector_ids = torch.arange(self.k, device=x.device)   # (k,)
        detector_ids = detector_ids.repeat(h*w)               # (h*w*k,)
        detector_ids = detector_ids.unsqueeze(0).expand(b, -1) # (b, seq_len)
        det_emb = self.detector_embedding(detector_ids)           # (b, seq_len, embed_dim)

        # row / col embedding
        row_ids = torch.arange(h, device=x.device).repeat_interleave(w*self.k)  # (h*w*k,)
        col_ids = torch.arange(w, device=x.device).repeat_interleave(self.k).repeat(h) # (h*w*k,)
        row_ids = row_ids.unsqueeze(0).expand(b, -1)  # (b, seq_len)
        col_ids = col_ids.unsqueeze(0).expand(b, -1)  # (b, seq_len)
        row_emb = self.row_embed(row_ids)             # (b, seq_len, embed_dim)
        col_emb = self.col_embed(col_ids)             # (b, seq_len, embed_dim)

        # 融合数值和缺失
        x = val_emb + mask_emb + det_emb + row_emb + col_emb   # (b, seq_len, embed_dim)
    ...
    with tqdm(range(config["epochs"]), desc=f"Epoch 0/{config['epochs']} - Loss: NaN", total=config["epochs"]) as pbar:
        for epoch in pbar:
            ...
            for x, y_cls, y_reg, mask_cls, mask_reg in train_loader:
                ...
                print(f"x.shape={x.shape},y_cls.shape={y_cls.shape}, y_reg.shape={y_reg.shape},"
                      f",mask_cls.shape={mask_cls.shape},mask_reg.shape={mask_reg.shape}")
                optimizer.zero_grad()
